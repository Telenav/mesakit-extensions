...
public class PbfWorldGraphExtractorApplication extends Application
{
    ...
    private final ArgumentParser<File> INPUT =
            File.fileArgument("The PBF file to split into a new world graph")
                    .required()
                    .build();

    public final SwitchParser<File> EXCLUDED_HIGHWAY_TYPES_FILE =
            File.fileSwitch("excluded-highway-types", "A text file containing excluded highway types (one per line)")
                    .optional()
                    .build();

    public final SwitchParser<File> FREE_FLOW_SIDE_FILE =
            File.fileSwitch("free-flow-side-file", "The file to load free flow from")
                    .optional()
                    .build();

    public final SwitchParser<File> INCLUDED_HIGHWAY_TYPES_FILE =
            File.fileSwitch("included-highway-types", "A text file containing included highway types (one per line)")
                    .optional()
                    .build();
    ...
    final SwitchParser<Count> THREADS = SwitchParser.threadCountSwitch(Count.count(24));
    ...
    public final SwitchParser<File> SPEED_PATTERN_FILE =
            File.fileSwitch("speed-pattern", "The speed pattern file to compile into each cell")
                    .optional()
                    .build();
